  ...
  return (
    <div
      className={`w-full h-[40px] ${
        isSelected ? "bg-[#ABD1F5]" : "bg-white"
      } hover:bg-[#ECF5FF]  shadow-md rounded-md flex flex-row gap-2 p-2 items-center justify-between`}
    >
      {!onEditorMode && (
        <button
      ...
      {onEditorMode && (
        <input
          ...
          className="w-full h-full bg-[#ECF5FF] rounded-md p-2"
      ...
      {onEditorMode && (
        <button
          ...
          className="bg-[#ABD1F5] rounded-md cursor-pointer"
        >
          <Save></Save>
        </button>
      )}
