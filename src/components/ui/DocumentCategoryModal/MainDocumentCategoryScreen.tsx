...
export const MainDocumentCategoryScreen = () => {
  ...
  const [categoriesData, setCategoriesData] = useState<
    CategoryFormatted[] | undefined
  >(undefined);
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryFormatted | null>(null);
  ...
  const handleUpdateCategory = (category: CategoryFormatted) => {
    const currentCategories = categoriesData?.filter(
      (cat) => !cat.isNewCategory
    );
  ...
  return (
    <div className="flex flex-row h-full gap-4 max-h-full overflow-y-auto">
      <div className="flex flex-col gap-4 h-full w-[300px] max-h-full overflow-y-auto">
        <div className=" h-full  flex flex-col gap-2 truncate  max-h-full overflow-y-auto">
          {categoriesData?.map((category) => (
            <CategoryItem
              onEditorMode={category.isNewCategory}
              isSelected={selectedCategory?.name == category.name}
              key={category.name}
              category={category}
              onClick={() => handleClickSelectCategory(category.name)}
